...
int game::evaluate(){
   ...
   char b_king_pos;
   int pos = 0;
   char w_bishop_count = 0;
   char b_bishop_count = 0;
	for(char i=0; i<64; i++) {
	  ...
	  else if (position->charBoard[i] == position->white_queens) {
		 w_piece_cnt++;
		 pos += 880;
		 }
	...
	if((b_piece_cnt + w_piece_cnt) < 16) {
	   pos += king_endgame_bonus_w[w_king_pos];
	   pos -= king_bonus_w[w_king_pos];
      pos -= king_endgame_bonus_b[b_king_pos];
      pos += king_bonus_b[b_king_pos];
	}
